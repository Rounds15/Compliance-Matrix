/* Shared pieces, ported from the design's components.js. Markup and class
   names are unchanged so the design's stylesheet applies as-is. */
...
export const initials = n => n ? n.split(/\s+/).filter(Boolean).slice(0, 2).map(w => w[0]).join("").toUpperCase() : "?";

export function Avatar({ person, size = 32, photo }) {
...
export const Risk = ({ r }) => <span className={"risk " + r}>{r}</span>;
...
const modalStack = [];
export function Modal({ children, onClose, size }) {
  ...
  return <div className="scrim" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
    <div className={"cm-modal" + (size === "sm" ? " sm" : "")} role="dialog" aria-modal="true">{children}</div>
  </div>;
}
...
export const Empty = ({ title, sub }) => <div className="empty"><h3>{title}</h3><p style={{ color: "#707780" }}>{sub}</p></div>;
